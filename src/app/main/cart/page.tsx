...
export default function CartPage() {
  ...
  const decreaseProduct = useCartStore((state) => state.removeFromCart)
  const clearCart = useCartStore((state) => state.clearCart)
  const removeProductFromCart = useCartStore(
    (state) => state.clearProductQuantityInCart
  )
                  ...
                  <div className="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                    <div className="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                      <div>
                        ...
                        <p className="mt-1 text-sm font-medium text-gray-900">
                          Количество в корзине: {product.quantityInCart}
                        </p>
                    ...
                    <p className="mt-4 flex space-x-2 text-sm text-gray-700">
                      {product.quantity !== 0 ? (
                        <>
                          <span>Количество на складе: {product.quantity}</span>
                          <CheckIcon
          ...
          >
            ...
            <dl className="mt-6 space-y-4">
              <div className="flex items-center justify-between">
                <dt className="text-sm text-gray-600">Итого за продукты: </dt>
